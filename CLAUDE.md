...
- `main` — integration branch. Merged features so far: Vault CRM design reskin, 3-column board, retro_ object prefix, drag/drop, action assignees, card grouping, group renaming, action item refinements, dashboard open-action count, refresh button, kudos column, native Vault tabs (Boards / Actions / Release Sentiment / Star Performers), `retro_release__c` + `retro_feature__c` + `retro_board_feature__c` data model, JIRA-style type-ahead pickers for release/feature, Release Sentiment chart + Sentiment-by-Feature matrix (scored `cards + votes`, keyed per release × team × feature, with a muted `(no feature)` roll-up chip), Star Performers podium with clickable-quote deep-links into the Boards tab, and mutation-clears-highlight behavior on BoardView so arriving-from-a-deep-link glow fades on the first edit.
...
- **Four Vault Pages**, one per tab dimension. All bind to the same controller; each has its own `page_client_code` so its own JS bundle loads:
  ...
  - `Page.retrovault_releases__c` (Release Sentiment, url `retrovault-releases`)
  - `Page.retrovault_teams__c` (Star Performers, url `retrovault-teams`)
- **Four Vault Tabs**: Boards (100), Actions (101), Release Sentiment (102), Star Performers (103). `url_path_name` cannot contain underscores — use dashes. The `retrovault-releases` / `retrovault-teams` URL paths are kept unchanged from their earlier "Releases" / "Teams" labels so existing deep links keep working.
...
- **React uses state-based routing** within the Boards entry (no React Router — Dashboard / BoardView / CreateBoard / SeedData); the Actions, Release Sentiment, and Star Performers entries are single-view shells.
...
cd client
npm install
npm run build         # bundles to dist/boards.js + dist/actions.js + dist/releases.js + dist/teams.js
npm run package       # creates retrovault.zip with all four entries + CSS
...
for P in \
    ...
    "retrovault_releases__c|Release Sentiment|retrovault-releases|retrovault_releases__c" \
...
for T in \
    ...
    "retrovault_releases__c|Release Sentiment|102|retrovault_releases__c" \
...
- `client/src/pages/BoardView.jsx` - Most complex page: 4-column layout (Kudos | Went Well | To Improve | Action Items), drag-and-drop, grouping, kudos modal variant. Feedback modal's feature dropdown is populated from the board's `retro_board_feature__c` junction rows (falls back to all features for the release if the board has no assignments yet). Accepts a generic `highlightId` prop (from the deep-link `#custom/page/retrovault/<boardId>/<cardId>`) that matches *either* an action card or a feedback card — both `ActionCard` and `FeedbackCard` support `highlighted` via the shared `vault-*-card--highlighted` CSS and scroll themselves into view. The highlight is seeded once from the prop into local state and cleared on the first user mutation (vote, create/edit/delete, drag drop, group / rename / ungroup) so it doesn't linger.
...
- `client/src/pages/Releases.jsx` - Two cards. Top: "Overall Sentiment by Release" multi-line SVG chart (one line per team across releases; sentiment = Went Well score / (Went Well + To Improve) score, where each card contributes `1 + vote_count__c` to its category). Bottom: "Sentiment by Feature" — a release × team matrix showing per-cell feature chips with `+N / −N` badges using the same `cards + votes` score; cards whose `feature__c` is blank roll up into a muted `(no feature)` chip per cell. Counts are keyed by `release | team | feature` so each cell reflects only its own team's board. Served via `retrovault_releases__c` Page / Tab.
- `client/src/pages/Teams.jsx` - Star Performers grouped per team. Each team section has a 3-card podium (gold/silver/bronze) and, if there are ≥4 recipients, a compact table for the rest. Ranking is by `totalVotes` desc, tiebreaker `kudosCount` desc. Each podium card surfaces the recipient's top-voted kudos quote; clicking that quote navigates the Vault top-frame to the Boards page with the kudos card as the `highlightId` (deep-link format `#custom/page/retrovault/<boardId>/<feedbackId>`). Served via `retrovault_teams__c` Page / Tab.
